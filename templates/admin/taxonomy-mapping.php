...
<section class="progress-agentic-rag__mapping" aria-labelledby="progress-agentic-rag-taxonomy-mapping-title">
	<div class="progress-agentic-rag__mapping-heading">
		<h3 id="progress-agentic-rag-taxonomy-mapping-title"><?php esc_html_e( 'Taxonomy label mapping', 'progress-agentic-rag' ); ?></h3>
		<p><?php esc_html_e( 'Map WordPress taxonomy terms to Progress Agentic RAG labels. Labelset suggestions come from your Knowledge Box.', 'progress-agentic-rag' ); ?></p>
	</div>
	<div>
		<?php if ( empty( $taxonomies ) ) : ?>
			<p class="progress-agentic-rag__mapping-muted"><?php esc_html_e( 'No public taxonomies available for mapping.', 'progress-agentic-rag' ); ?></p>
		<?php else : ?>
			<?php $mapped_taxonomies = array_keys( $taxonomy_label_map ); ?>
			<div class="progress-agentic-rag__mapping-control">
				<label for="progress-agentic-rag-taxonomy-select"><?php esc_html_e( 'Add taxonomy mapping:', 'progress-agentic-rag' ); ?></label>
				<select id="progress-agentic-rag-taxonomy-select" data-progress-agentic-rag-taxonomy-select>
					<option value=""><?php esc_html_e( 'Select a taxonomy', 'progress-agentic-rag' ); ?></option>
				...
				<button type="button" class="progress-agentic-rag__button progress-agentic-rag__button--secondary" data-progress-agentic-rag-add-mapping><?php esc_html_e( 'Add mapping', 'progress-agentic-rag' ); ?></button>
			</div>
				...
				<?php foreach ( $taxonomy_label_map as $taxonomy_key => $config ) : ?>
					...
					<div class="progress-agentic-rag__mapping-block" data-progress-agentic-rag-mapping-block data-taxonomy="<?php echo esc_attr( $taxonomy_key ); ?>">
						<div class="progress-agentic-rag__mapping-block-header">
							...
							<button type="button" class="progress-agentic-rag__button progress-agentic-rag__button--danger" data-progress-agentic-rag-remove-mapping><?php esc_html_e( 'Remove', 'progress-agentic-rag' ); ?></button>
						</div>
						<label class="progress-agentic-rag__mapping-select" for="progress-agentic-rag-labelset-<?php echo esc_attr( $taxonomy_key ); ?>">
							<span><?php esc_html_e( 'Labelset', 'progress-agentic-rag' ); ?></span>
							<select id="progress-agentic-rag-labelset-<?php echo esc_attr( $taxonomy_key ); ?>" name="<?php echo esc_attr( $taxonomy_label_map_option ); ?>[<?php echo esc_attr( $taxonomy_key ); ?>][labelset]" data-progress-agentic-rag-labelset-select data-taxonomy="<?php echo esc_attr( $taxonomy_key ); ?>">
								<option value=""><?php esc_html_e( 'Select a labelset', 'progress-agentic-rag' ); ?></option>
						...
						<?php if ( empty( $labelsets ) ) : ?>
							<p class="progress-agentic-rag__mapping-muted"><?php esc_html_e( 'No labelsets available. Check your Progress Agentic RAG credentials.', 'progress-agentic-rag' ); ?></p>
						<?php endif; ?>

						<?php if ( empty( $terms ) ) : ?>
							<p class="progress-agentic-rag__mapping-muted"><?php esc_html_e( 'No terms available for this taxonomy.', 'progress-agentic-rag' ); ?></p>
						<?php else : ?>
							<table class="progress-agentic-rag__mapping-table">
								<thead>
									<tr>
										<th scope="col"><?php esc_html_e( 'Term', 'progress-agentic-rag' ); ?></th>
										<th scope="col"><?php esc_html_e( 'Progress Agentic RAG labels', 'progress-agentic-rag' ); ?></th>
									</tr>
								</thead>
								<tbody>
											...
											<td>
												<div class="progress-agentic-rag__label-checkboxes" data-progress-agentic-rag-label-checkboxes data-taxonomy="<?php echo esc_attr( $taxonomy_key ); ?>" data-term-id="<?php echo esc_attr( (string) $term->term_id ); ?>">
													<?php if ( empty( $available_labels ) ) : ?>
														<em><?php echo '' === $taxonomy_labelset ? esc_html__( 'Select a labelset to load labels.', 'progress-agentic-rag' ) : esc_html__( 'No labels available.', 'progress-agentic-rag' ); ?></em>
													<?php else : ?>
						...
						<div class="progress-agentic-rag__fallback">
							<p><strong><?php esc_html_e( 'Fallback labels (when no terms assigned)', 'progress-agentic-rag' ); ?></strong></p>
							...
							<div class="progress-agentic-rag__fallback-labels" data-progress-agentic-rag-fallback-labels data-taxonomy="<?php echo esc_attr( $taxonomy_key ); ?>">
								<?php if ( empty( $fallback_available ) ) : ?>
									<em><?php echo '' === $fallback_labelset ? esc_html__( 'Select a labelset to load labels.', 'progress-agentic-rag' ) : esc_html__( 'No labels available.', 'progress-agentic-rag' ); ?></em>
								<?php else : ?>
